...
for_creating_segments_from_strava <- telemetry_available %>%
  ...
  filter(!activity_id %in% c("5028077839",
                             "5063406337",
                             "5125903648",
                             "5485415918",
                             "5026068681",
...
RRR <- segments_from_fr_data %>%
  ...
  inner_join(telemetry_available %>%
               anti_join(dbGetQuery(con, "SELECT DISTINCT activity_id FROM strava_new_segment_creation_interim
...
for(g in 1:length(RRR$race)) {
  ...
  link_with_correct_segments <- segments_from_fr_data %>%
    ...
    rbind(segments_from_fr_data %>%
            ...
            mutate(start_prior = end_km,
                   end_next = stage_length) %>%
            filter(end_next > start_prior)) %>%
    filter(end_next > start_prior) %>%
    
    arrange(start_prior) %>%
    
    mutate(start_lat1 = ifelse(rowid == 1, vstart_lat, lag(end_lat)),
           start_long1 = ifelse(rowid == 1, vstart_long, lag(end_long))) %>%
    
    mutate(end_lat = ifelse(rowid == max(rowid), vfinish_lat, start_lat),
           end_long = ifelse(rowid == max(rowid), vfinish_long, start_long)) %>%
    
    select(-start_lat, -start_long) %>%
    
    rename(start_lat = start_lat1,
           start_long = start_long1) %>%
    
    select(-rowid) %>%
    
    select(start_lat, start_long, end_lat, end_long,
           stage, race, year, date, class, start_prior, end_next, creation_type) %>%
