...
all_stage_data <- dbGetQuery(con, "SELECT * FROM stage_data_perf WHERE year IN (2023)") %>%
...
for_creating_segments_from_strava <- dbReadTable(con, "ideal_activity_id_for_race") %>%
  anti_join(dbGetQuery(con, "SELECT DISTINCT stage, race, year FROM segments_from_strava_data") %>%
...
for(g in 1:length(for_creating_segments_from_strava$race)) {
  ...
  if(length(lat) == 0) {
    ...
    smooth_route <- all_routes %>%
      ...
      mutate(every_km2 = floor(left_km/0.2)/5) %>%
      
      group_by(every_km2) %>%
      filter(distances == min(distances) | distances == max(distances)) %>%
      mutate(start_alt = ifelse(distances == min(distances), elevations, NA),
             end_alt = ifelse(distances == max(distances), elevations, NA)) %>%
      summarize(
        start_km = max(left_km, na.rm = T),
        end_km = min(left_km, na.rm = T),
        start_alt = mean(start_alt, na.rm = T),
        end_alt = mean(end_alt, na.rm = T),
      ...
      mutate(alt_traveled = lead(start_alt) - start_alt,
             dist_traveled = (start_km - lead(start_km)) * 1000,
             dist_traveled = ifelse(end_km == 0, start_km * 1000, dist_traveled),
             alt_traveled = ifelse(end_km == 0, end_alt - start_alt, alt_traveled),
             gradient = alt_traveled / dist_traveled) %>%
      ...
      mutate(change_gradient = ifelse(is.na(change_gradient), "flat", change_gradient)) %>%
      ...
      group_by(grouping) %>%
      mutate(grouping = ifelse(n() == 1, NA, grouping)) %>%
      ungroup() %>%
      
      mutate(change_gradient = ifelse(is.na(grouping), NA, change_gradient)) %>%
      
      mutate(change_gradient = ifelse(is.na(grouping),
                                      ifelse(lag(change_gradient) == lead(change_gradient), lead(change_gradient), NA), change_gradient)) %>%
      
      mutate(change_gradient = ifelse(is.na(change_gradient), "mixed", change_gradient)) %>%
      
      select(-c(rowname, grouping, rk)) %>%
      ...
      group_by(grouping) %>%
      mutate(total_dist_traveled = sum(dist_traveled, na.rm = T),
             total_alt_traveled = sum(alt_traveled, na.rm = T)) %>%
      ungroup() %>%
      
      mutate(total_gradient = total_alt_traveled / total_dist_traveled)
    
    #
    
    next_smooth <- smooth_route %>% 
      
      mutate(grouping = ifelse(change_gradient == "mixed", rk, grouping)) %>%
      
      group_by(grouping, change_gradient) %>% 
      mutate(start_lat = ifelse(min(rowname) == rowname, lat, NA), 
             end_lat = ifelse(max(rowname) == rowname, lat, NA),
             start_long = ifelse(min(rowname) == rowname, long, NA), 
             end_long = ifelse(max(rowname) == rowname, long, NA),
             start_km = ifelse(min(rowname) == rowname, start_km, NA),
             end_km = ifelse(max(rowname) == rowname, end_km, NA),
             start_alt = ifelse(min(rowname) == rowname, start_alt, NA),
             end_alt = ifelse(max(rowname) == rowname, end_alt, NA)) %>%
      ungroup() %>%
      
      group_by(grouping, change_gradient) %>%
      summarize(start_lat = mean(start_lat, na.rm = T),
      ...
      group_by(grouping) %>%
      mutate(grouping = ifelse(dist_traveled < 250, NA, grouping)) %>%
      ungroup() %>%
    ...
    ups_and_downs <- next_smooth %>% 
      filter(change_gradient %in% c("uphill", "downhill") & dist_traveled >= 500)
    
    #
    
    different_groupings_uphill <- ups_and_downs %>%
      
      filter(change_gradient == "uphill") %>%
      
      rename(gain = alt_traveled,
             length = dist_traveled,
             start_elev = start_alt,
             end_elev = end_alt) %>%
      
      filter(length > 0) %>%
      filter(gradient > 0.025)
    
    different_groupings_downhill <- ups_and_downs %>%
    ...
    if(length(different_groupings_downhill$grouping) > 0 | length(different_groupings_uphill$grouping) > 0) {
      ...
      segments <- rbind(different_groupings_downhill, different_groupings_uphill) %>%
        select(-c(grouping, rk, total_dist_traveled, total_alt_traveled, total_gradient, rowname,
                  change_gradient)) %>%
        ...
        mutate(grouping = rank(desc(start_km), ties.method = "first"))
...
# 250m Segments Whole Race ------------------------------------------------

for_creating_segments_from_strava <- dbReadTable(con, "ideal_activity_id_for_race") %>%
  anti_join(dbGetQuery(con, "SELECT DISTINCT stage, race, year FROM strava_250m_segment_creation") %>%
              mutate(stage = as.character(stage))) %>%
  
  inner_join(telemetry_available %>% select(activity_id)) %>%
  filter(activity_id != '8388867341') %>%
  arrange(desc(class %in% c("1.UWT", "2.UWT", "WC", "Olympics")), desc(date)) %>%
  
  filter(!(stage == 1 & race == 'tour de romandie' & year == 2019)) %>%
  filter(!(stage == 4 & race == 'voo-tour de wallonie' & year == 2019)) %>%
  filter(!(stage == 3 & race == 'tour de korea' & year == 2018)) %>%
  unique() %>%
  group_by(pcs, stage, race, year, class, date) %>% 
  filter(n() == 1) %>%
  ungroup()
...
RRR <- for_creating_segments_from_strava %>%
  ...
  inner_join(telemetry_available %>%
               anti_join(dbGetQuery(con, "SELECT DISTINCT activity_id FROM strava_250m_segment_creation
                                    WHERE creation_type = 'TIME'")) %>%
               select(stage, race, year, class, date) %>%
               group_by(stage, race, year, class, date) %>%
               count() %>%
...
for(g in 1:length(RRR$race)) {
  ...
  correct_stage_info <- for_creating_segments_from_strava %>%
  ...
  if(length(data_lists$distance) == 0) {} else {
    ...
    link_with_correct_segments <- all_routes %>%
      
      mutate(x250m = floor(left_km / 0.25) * 0.25) %>%
      
      group_by(x250m) %>%
      mutate(start_lat = ifelse(left_km == max(left_km, na.rm = T), lat, NA),
             start_long = ifelse(left_km == max(left_km, na.rm = T), long, NA),
             end_lat = ifelse(left_km == min(left_km, na.rm = T), lat, NA),
             end_long = ifelse(left_km == min(left_km, na.rm = T), long, NA)) %>%
      
      summarize(start_lat = median(start_lat, na.rm = T),
                start_long = median(start_long, na.rm = T),
                end_lat = median(end_lat, na.rm = T),
                end_long = median(end_long, na.rm = T),
                start_km = max(left_km, na.rm = T),
                end_km = min(left_km, na.rm = T)) %>%
      ungroup() %>%
      ...
      mutate(start_lat = ifelse(rowid == max(rowid), start_lat, lead(end_lat)),
             start_long = ifelse(rowid == max(rowid), start_long, lead(end_long)),
             start_km = ifelse(rowid == max(rowid), start_km, lead(end_km))) %>%
      
      mutate(should_be_distance = start_km - end_km) %>%
      
      mutate(start_prior = max(start_km) - start_km,
             end_next = max(start_km) - end_km) %>%
      
      select(-x250m, -start_km, -end_km) %>%
      
      cbind(for_creating_segments_from_strava %>%
              
              filter(race == R &
                       stage == S &
                       year == Y)) %>%
      select(-pcs, -distance, -length, -activity_id, -stage_type)
    ...
    if(nrow(bind_rows(new_segments_list2)) == 0) {} else {
      ...
      dbWriteTable(con, "strava_250m_segment_creation", all_new_segments, append = TRUE, row.names = F)
...
races_to_pull_in <- dbGetQuery(con, "SELECT DISTINCT stage, race, year, date FROM strava_250m_segment_creation") %>%
  
  anti_join(dbGetQuery(con, "SELECT DISTINCT stage, race, year, date FROM position_in_race_synthetic_segments"))

#

for(A in 1:nrow(races_to_pull_in)) {
  
  position_in_race <- dbGetQuery(con, sprintf("SELECT * FROM strava_250m_segment_creation
           WHERE date = '%s' AND stage = '%s' AND year = '%s'",
                                              races_to_pull_in$date[[A]], races_to_pull_in$stage[[A]],
                                              races_to_pull_in$year[[A]])) %>%
    filter(race == races_to_pull_in$race[[A]]) %>%
    inner_join(telemetry_available %>% select(activity_id, rider = pcs, rnk, weight), by = c("activity_id")) %>% 
    filter(segment_distance >= 220 & segment_distance <= 280) %>%
    
    arrange(end_next) %>%
    
    group_by(rowid) %>%
    mutate(avg_time = mean(segment_time, na.rm = T)) %>%
    ungroup() %>%
    
    group_by(rider) %>%
    mutate(time_vs_avg = cummean(avg_time - segment_time)) %>%
    ungroup() %>%
    
    filter(end_next >= 0) %>%
    
    select(race, stage, year, class, date, KM_raced = end_next, activity_id, rider, rnk, time_vs_avg) %>%
    
    mutate(KM_raced = plyr::round_any(KM_raced, accuracy = 0.25))
  
  #
  
  written_segments <- dbGetQuery(con, "SELECT * FROM climbs_from_strava_telemetry") %>%
    
    inner_join(telemetry_available %>%
                 select(pcs, stage, race, year, class, date, activity_id, url, distance, race_length = length),
               by = c("stage", "race", "year", "url", "rider" = "pcs")) %>%
    mutate(segment_end = plyr::round_any(perc_thru * distance, accuracy = 0.25)) %>%
    
    inner_join(
      dbGetQuery(con, "SELECT DISTINCT rider, stage, url FROM climbs_from_strava_telemetry") %>%
        
        group_by(stage, url) %>%
        filter(rank(rider, ties.method = "first") == 1) %>%
        ungroup(), by = c("rider", "stage", "url"))
  
  
  #
  
  write_these_start_end <- position_in_race %>%
    ...
    inner_join(
      ...
      rbind(
        written_segments %>%
          ...
          rename(segment_km = segment_end) %>%
          mutate(segment_km = segment_km - length) %>%
          select(-rider, -gain, -model_category, -perc_thru, -power_required, -power_model_category, -activity_id, -distance, -race_length),
        written_segments %>%
          mutate(start_end = "END") %>%
          rename(segment_km = segment_end) %>%
          mutate(segment_km = segment_km) %>%
          select(-rider, -gain, -model_category, -perc_thru, -power_required, -power_model_category, -activity_id, -distance, -race_length)
      ), by = c("stage", "race", "year", "class", "date", "KM_raced" = "segment_km")
    )
  
  #
  
  dbWriteTable(con, "position_in_race_synthetic_segments", write_these_start_end, append = TRUE, row.names = FALSE)
...
x250m <- dbGetQuery(con, "SELECT * FROM strava_250m_segment_creation WHERE date = '2022-07-08'") %>%
...
x250m %>%
  inner_join(telemetry_available %>% select(activity_id, rider = pcs, rnk, weight), by = c("activity_id")) %>% 
  filter(ValidPower == ValidPoints & should_be_distance > 225) %>%
  ...
  group_by(rowid, end_next, GRP = time_vs_avg > 0.01) %>% 
  summarize(wattskg = mean(Power / weight), 
            gradient = mean(segment_vertgain/segment_distance)) %>%
  ungroup() %>% 
  
  ggplot(aes(x = 130-end_next, y = wattskg, color = GRP))+
  geom_line(size=1)+
  scale_x_reverse()
...
x250m %>% 
  group_by(activity_id, stage, race, year, class, date) %>% 
  arrange(start_prior) %>%
  mutate(Time = cumsum(segment_time), 
         D = cumsum(segment_distance), 
         n = cumsum(rowid > 0)) %>%
  ungroup() %>% 
  
  inner_join(telemetry_available %>% select(activity_id, rnk, rider = pcs), by = c("activity_id")) %>%
  mutate(kmh = D / (Time/3600)) -> cumul
...
cor_by_time <- cumul %>%
  ...
  group_by(stage, race, year, class, date, n) %>%
  summarize(cor = cor(x = kmh, y = rnk, use = 'complete.obs'),
            files = n()) %>%
  ungroup() %>%
  
  group_by(stage, race, year, class, date) %>%
  mutate(perc_thru = n / max(n)) %>%
  ungroup()
...
x250m %>%
  ...
  ggplot(aes(x = end_next, y = time_vs_avg, color = rider))+
  geom_line(size=1)+
  scale_x_continuous(breaks = seq(0,200,20))+
  coord_cartesian(xlim = c(20,170))
...
x250m %>%
  ...
  filter(between(end_next, 168.5, 169)) %>%
  
  select(rider, end_next, time_vs_avg) -> bottom_spandelles
...
x250m %>%
  ...
  filter(between(end_next, 130, 131)) %>%
  
  select(rider, end_next, time_vs_avg) -> bottom_hautacam
