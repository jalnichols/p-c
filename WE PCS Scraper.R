...
pull_from_schedule <- c(
  ...
  # WORLD TOUR
  'https://www.procyclingstats.com/races.php?year=2021&circuit=24&class=&filter=Filter',
  
  'https://www.procyclingstats.com/races.php?year=2021&circuit=&class=1.WWT&filter=Filter',
  
  'https://www.procyclingstats.com/races.php?year=2021&circuit=&class=2.WWT&filter=Filter',  
  
  # Women Elite
  
  'https://www.procyclingstats.com/races.php?year=2021&circuit=16&class=1.1&filter=Filter',
  
  'https://www.procyclingstats.com/races.php?year=2021&circuit=16&class=2.1&filter=Filter',
  
  'https://www.procyclingstats.com/races.php?year=2021&circuit=16&class=1.Pro&filter=Filter',
  
  'https://www.procyclingstats.com/races.php?year=2021&circuit=16&class=2.Pro&filter=Filter',
  
  'https://www.procyclingstats.com/races.php?year=2021&circuit=16&class=CC&filter=Filter',
  
  'https://www.procyclingstats.com/races.php?year=2021&circuit=26&class=&filter=Filter',
  
  # OLYMPICS AND WORLD CHAMPS
  
  'https://www.procyclingstats.com/races.php?year=2021&circuit=3&class=&filter=Filter',
  
  'https://www.procyclingstats.com/races.php?year=2021&circuit=2&class=&filter=Filter')
...
start_year = 2015 # set to 2012 to pull 2013, 2019 to pull 2020
    ...
    # if not current year, handle the same
      ...
      events <- cbind(
        ...
        page %>%
          ...
          filter(str_detect(value, "race/")) %>%
          mutate(value = ifelse(value == "race/african-continental-championships-we-itt/2020", 
                                "race/african-continental-championships-we-itt/2021", value)) %>%
          filter(str_detect(value, as.character(year)))
...
extra_races <- c("race/world-championship-we",
                 "race/world-championship-itt-we",
                 "race/olympic-games-we",
                 "race/olympic-games-we-itt",
                 "race/uec-road-european-championships-we",
                 "race/uec-road-european-championships-we-itt")
...
new_events <- all_events %>%
  anti_join(
    ...
    dbReadTable(con, "pcs_all_races_we") %>%
      filter(type != "scraper") %>%
...
dbWriteTable(con, "pcs_all_races_we", all_events, row.names = FALSE, append = TRUE)
...
dbWriteTable(con, "pcs_gc_winners_we", gc_winners %>% filter(!is.na(gc_winner)), append = TRUE, row.names = FALSE)
...
all_stages <- all_events %>%
  ...
  group_by(value, year) %>%
  filter(rank(Race, ties.method = "first") == 1) %>%
  ungroup() %>%
...
dbWriteTable(con, "pcs_all_stages_we", all_stages, append = TRUE, row.names = FALSE)
...
html_stage_dir <- fs::dir_ls("D:/Jake/Documents/PCS-HTML-we/")

# download new stages (TRUE) or use old HTML (FALSE)
dl_html <- TRUE

# races from this year
dbGetQuery(con, "SELECT stage, race, year, date FROM pcs_stage_raw_we WHERE rnk = '1'") -> Y21

# start scraping process using old data

if(dl_html == FALSE) {
  
  all_stages <- dbReadTable(con, "pcs_all_stages_we") %>%
    filter(year < 2020)
...
for(r in 1:length(all_stages$value)) {
  ...
  f_name <- paste0("D:/Jake/Documents/PCS-HTML-we/", str_replace_all(str_replace(all_stages$value[[r]], "https://www.procyclingstats.com/race/", ""), "/", ""))
  ...
  if(f_name %in% html_stage_dir | dl_html == TRUE) {
    ...
    if((str_detect(race_url, "race/hammer-") == TRUE)) {
...
#dbSendQuery(con, "DELETE FROM pcs_stage_raw_we")

#dbWriteTable(con, "pcs_stage_raw_we", test_dl, append = TRUE, row.names = FALSE)
...
stage_data_raw <- dbReadTable(con, "pcs_stage_raw_we")
...
pcs_all_races <- dbGetQuery(con, "SELECT DISTINCT url, type FROM pcs_all_races_we") %>%
...
winners <- dbReadTable(con, "pcs_all_races_we") 
...
stage_data <- stage_data %>%
  ...
  mutate(missing_profile_data = TRUE) %>%
...
y22 <- stage_data %>% filter(rnk == 1)

#dbSendQuery(con, "DELETE FROM pcs_stage_data")

dbWriteTable(con, "pcs_stage_data_we", 
